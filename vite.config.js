  ...
  return defineConfig({
    plugins: [
      ...
      viteVConsole({
        ...
        // 仅在非生产环境并且通过 env 打开时启用 vconsole
        localEnabled: !isProd && VITE_OPEN_VCONSOLE,
        enabled: !isProd && VITE_OPEN_VCONSOLE,
        config: {
          maxLogNumber: 1000,
    ...
    server: {
      ...
      proxy: {
        "/api": {
          target: "http://116.62.102.235:80",
          changeOrigin: true,
          pathRewrite: {
            "^/api": "", // 路径重写，去掉api
          },
          // rewrite: (path) => path.replace(/^\/api/, ''), // 路径重写，去掉api
        },
        "/baidu-api": {
          target: "https://aip.baidubce.com",
          changeOrigin: true,
          secure: true,
          rewrite: (path) => path.replace(/^\/baidu-api/, ""), // 路径重写，去掉baidu-api前缀
        },
      },
    },
    build: {
